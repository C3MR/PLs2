...
CREATE TABLE public.lead_activities (
  ...
  -- Contact Details
  contact_method TEXT, -- 'phone', 'email', 'in_person', 'whatsapp'
...
CREATE POLICY "Assigned agents can manage pipeline" ON public.sales_pipeline
  FOR ALL USING (
...
CREATE POLICY "Users can view assigned tasks" ON public.crm_tasks
  FOR SELECT USING (
    auth.uid() = assigned_to OR 
    auth.uid() = created_by OR
...
CREATE INDEX idx_pipeline_agent ON public.sales_pipeline(assigned_agent);
CREATE INDEX idx_pipeline_close_date ON public.sales_pipeline(expected_close_date);
