-- إزالة القيد الأجنبي وإضافة العقارات
...
-- إضافة عمود property_code لتسهيل الربط
ALTER TABLE public.properties ADD COLUMN IF NOT EXISTS property_code TEXT UNIQUE;

-- تحديث العقارات بأكواد فريدة
UPDATE public.properties SET property_code = 'PROP-' || LPAD((ROW_NUMBER() OVER (ORDER BY created_at))::TEXT, 3, '0') WHERE property_code IS NULL;